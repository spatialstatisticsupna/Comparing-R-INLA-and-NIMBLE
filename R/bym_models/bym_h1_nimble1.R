...
bym.type1.h1 <- nimbleCode({
  ...
  for(i in 1:N){ 
    for (k in 1:t){
      ...
      log(lambda[i,k]) <- log(E[i,k]) + alpha0 + sd.u*spat.u[i] + sd.v*spat.v[i] + sd.temp*temp[k] + stType1.centered[i,k]
...
bym.type2.h1 <- nimbleCode({
  ...
  for(i in 1:N){ 
    for (k in 1:t){
      y[i,k] ~ dpois(lambda[i,k])
      log(lambda[i,k]) <- log(E[i,k]) + alpha0 + sd.u*spat.u[i] + sd.v*spat.v[i] + sd.temp*temp[k] + sd.t2*stType2[i,k]
...
bym.type3.h1 <- nimbleCode({
  ...
  for(i in 1:N){ 
    for (k in 1:t){
      y[i,k] ~ dpois(lambda[i,k])
      log(lambda[i,k]) <- log(E[i,k]) + alpha0 + sd.u*spat.u[i] + sd.v*spat.v[i] + sd.temp*temp[k] + sd.t3*stType3[i,k]
...
bym.type4.h1 <- nimbleCode({
  ...
  for(i in 1:N) {
    for (k in 1:t){ 
      y[i,k] ~ dpois(lambda[i,k])
      log(lambda[i,k]) <- log(E[i,k]) + alpha0 + sd.u*spat.u[i] + sd.v*spat.v[i] + sd.temp*temp[k] + sd.t4*stType4[i,k]
