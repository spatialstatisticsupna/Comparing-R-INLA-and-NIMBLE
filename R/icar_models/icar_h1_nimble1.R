...
icar.type1.h1 <- nimbleCode({
  ...
  for(i in 1:N){ 
    for (k in 1:t){
      ...
      y[i,k] ~ dpois(mu[i,k])
      log(mu[i,k]) <- log(E[i,k]) + alpha0 + sd.u*spat.u[i] + sd.temp*temp[k] + stType1.centered[i,k]
...
icar.type2.h1 <- nimbleCode({
  ...
  for(i in 1:N){ 
    for (k in 1:t){
      y[i,k] ~ dpois(mu[i,k])
      log(mu[i,k]) <- log(E[i,k]) + alpha0 + sd.u*spat.u[i] + sd.temp*temp[k] + sd.t2*stType2[i,k]
...
icar.type3.h1 <- nimbleCode({
  ...
  for(i in 1:N){ 
    for (k in 1:t){
      y[i,k] ~ dpois(mu[i,k])
      log(mu[i,k]) <- log(E[i,k]) + alpha0 + sd.u*spat.u[i] + sd.temp*temp[k] + sd.t3*stType3[i,k]
...
icar.type4.h1 <- nimbleCode({
  ...
  for(i in 1:N) {
    for (k in 1:t){ 
      y[i,k] ~ dpois(mu[i,k])
      log(mu[i,k]) <- log(E[i,k]) + alpha0 + sd.u*spat.u[i] + sd.temp*temp[k] + sd.t4*stType4[i,k]
  ...
  for(k in 1:t){
    Etemp[k] <- exp(sd.temp*temp[k])
    Ealpha0.temp[k] <- exp(alpha0 + sd.temp*temp[k])
  }
